...
public class HistoryWriter {
	...
	public static void writeReadlineHistory(ConsoleReader console, final OutputStream out) throws IOException {
		final PrintWriter w = new PrintWriter(new OutputStreamWriter(out, "UTF-8"));
		final int len = console.getHistory().size();
		for (int i = 0; i < len; ++i) {
			String line = (String) console.getHistory().getHistoryList().get(i);
			if (line == null) {
				continue;
			}
			line = escape(line);
	...
	public static void readReadlineHistory(ConsoleReader console, final InputStream in) throws IOException {
		// todo: check first utf-8, then default-encoding to be
		// backward-compatible.
		readReadlineHistory(console, new InputStreamReader(in, "UTF-8"));
	}

	private static void readReadlineHistory(ConsoleReader console, final Reader in) throws IOException {
		...
		do {
			...
			if (line.length() > 0) {
				console.getHistory().addToHistory(line.toString());
				line.setLength(0);
			}
		} while (c >= 0);
		r.close();
